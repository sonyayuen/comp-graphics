...
void render_scene() {
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      float d = (float) (1.0 / tan(radians(daFOV) / 2.0)) ;
      float U = (float)  ((2.0 * float(x)) / float(width) - 1.0);
      float V =  (float)(((2.0 * float(y)) / float(height) - 1.0) * -1.0); //mult by -1 to invery y-coord
      ...
      PVector first = PVector.mult(uvw[2], -d);
      PVector second = PVector.mult(uvw[1], V);
      PVector third = PVector.mult(uvw[0], U);
      
      
      PVector firstAndSec = PVector.add(first, second);
      dirVec = PVector.add(firstAndSec, third) ;
      
      dirVec.normalize();
      
      // Create a ray using eyeVec and dirVec.
      Ray currRay = new Ray(eyeVec, dirVec); //did this
      if (debug_flag) {
        println("");
...
Hit rayIntersectScene(Ray ray) {
  ...
  float minT = 50000;  // Some large number "i init minT to big num"
  ...
  for (int i = 0; i < shapes.size(); i++) { //returns the closest hit which is minHit
    Hit temp = shapes.get(i).intersectRay(ray) ;
    
    if ( temp != null && temp.t < minT) {
      minHit = temp;
    } 
  }
...
PVector shadeHelper(Hit sceneHit) {
  ...
  for (Light currLight: lights) {
    ...
    float diffuseCoeff = max(0, N.dot(L));
    
    totalCol.x += (currShapeDifCol.x * currLight.col.x * diffuseCoeff);
    totalCol.y += (currShapeDifCol.y * currLight.col.y * diffuseCoeff);
    totalCol.z += (currShapeDifCol.z * currLight.col.z * diffuseCoeff);
