...
PVector shadeHelper(Hit sceneHit, int depth) {
  ...
  if (Debug.DO_AMBIENT){
    // (TODO) update the total color using the given ambient color: 
    //sceneHit.hitShape.mat.ambColor 
  ...
  if (Debug.DO_REFLECTION && depth > 0 && sceneHit.hitShape.mat.kRefl > 0){
    ...
    PVector negD = PVector.mult(sceneHit.ray.direction, -1); //-d
    float nDotd = PVector.dot(N, negD) ; //(2 * (N dot (-d)))
    nDotd *= 2;
    
    PVector R = PVector.add(sceneHit.ray.direction, PVector.mult(N, nDotd));
    R.normalize();
  ...
  for (Light currLight: lights) {
    ...
    PVector diffuseContribution = PVector.mult(
      pairwiseMult(currLight.col, sceneHit.hitShape.mat.difColor),diffuseFactor);
